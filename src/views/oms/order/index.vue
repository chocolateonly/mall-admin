  ...
  getOrderDetail(row.id)
    .then((response: any) => {
      if (response.data && typeof response.data === "object") {
        // 处理订单商品信息，确保字段正确映射
        const orderItems = response.data.orderItems || response.data.order?.orderItems || [];
        ...
        state.orderDetail = {
          ...
          order: {
            ...
            ...(response.data.order || {}),
            orderSn: response.data.orderSn || response.data.order?.orderSn || '',
            memberId: response.data.memberId || response.data.order?.memberId || '',
            source: response.data.source !== undefined ? response.data.source : (response.data.order?.source || 0),
            status: response.data.status !== undefined ? response.data.status : (response.data.order?.status || 0),
            totalAmount: response.data.totalAmount || response.data.order?.totalAmount || 0,
            paymentAmount: response.data.paymentAmount || response.data.order?.paymentAmount || 0,
            paymentMethod: response.data.paymentMethod || response.data.order?.paymentMethod || 0,
            createTime: response.data.createTime || response.data.order?.createTime 
              ? dayjs(response.data.createTime || response.data.order?.createTime).format("YYYY/MM/DD HH:mm:ss")
              : ""
          },
  ...
  <div class="app-container">
    ...
    <el-table ref="dataTable" v-loading="loading" :data="orderList" border>
          ...
          <el-table :data="scope.row.orderItems" border>
            ...
            <el-table-column label="图片" prop="picUrl">
              <template #default="slotScope">
                <img :src="slotScope.row.picUrl" width="40" />
              </template>
            </el-table-column>
            <el-table-column align="center" label="单价" prop="price">
              <template #default="slotScope">{{ slotScope.row.price }}</template>
            </el-table-column>
            <el-table-column align="center" label="数量" prop="quantity">
              <template #default="slotScope">{{ slotScope.row.quantity }}</template>
            </el-table-column>
          </el-table>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="订单来源">
        <template #default="scope">
          <el-tag>{{ orderSourceMap[scope.row.sourceType] || "未知来源" }}</el-tag>
        </template>
      </el-table-column>

      <el-table-column align="center" label="订单状态">
        <template #default="scope">
          <el-tag>{{ orderStatusMap[scope.row.status] || "未知状态" }}</el-tag>
        </template>
      </el-table-column>

      <el-table-column align="center" prop="totalAmount" label="订单金额">
        <template #default="scope">
      ...
      <el-table-column align="center" label="支付方式">
        <template #default="scope">
          <el-tag>{{ payTypeMap[scope.row.payType] || "未知方式" }}</el-tag>
        </template>
      </el-table-column>
    ...
    >
      <div>
        ...
        <el-table :data="state.orderDetail.orderItems" border>
          ...
          <el-table-column align="center" label="小计" prop="totalAmount">
